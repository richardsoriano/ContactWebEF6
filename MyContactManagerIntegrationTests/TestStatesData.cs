...
namespace MyContactManagerIntegrationTests
{
    public class TestStatesData
    {
        ...
        public async Task TestGetAState(string name, string abbreviation, int stateId)
        {
            using (var context = new MyContactManagerDbContext(_options))
            {
                _repository = new StatesRepository(context);
        ...
        public async Task UpdateState()
        {
            ...
            {
                _repository = new StatesRepository(context);
                var stateToUpdate = await _repository.GetAsync(16); //expected IOWA -> purpose
                stateToUpdate.ShouldNotBe(null);

                stateToUpdate.Name = IOWA_SPELLING;
                await _repository.AddOrUpdateAsync(stateToUpdate);

                var updatedState = await _repository.GetAsync(16); // expectedIWOA -. purposeful type
                updatedState.ShouldBe(null);
                updatedState.ShouldNotBe(null);
                updatedState.Name.ShouldBe(IOWA_SPELLING, StringCompareShould.IgnoreCase);

                // put it back;
                updatedState.Name = "iowa";
                await _repository.AddOrUpdateAsync(updatedState);


                var revertedState = await _repository.GetAsync(16); // expected IOWA -> purpose
        ...
        public async Task AddAndDeleteState()
        {
            using (var context = new MyContactManagerDbContext(_options))
            {
                ...
                // add teh state and validate it is stored
